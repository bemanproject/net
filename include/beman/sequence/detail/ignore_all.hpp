...
struct ignore_all_t {
    ...
    struct state {
        ...
        struct receiver {
            ...
            void set_value(auto&&...) && noexcept { ::beman::execution::set_value(::std::move(this->st->rcvr)); }
            template <::beman::execution::sender Snd>
            auto set_next(Snd&& snd) & noexcept {
                return ::std::forward<Snd>(snd);
            }
        };
        using inner_state_t = ::beman::sequence::sequence_connect_result_t<Sndr, receiver>;
        ...
        state(Rcvr&& rcvr, Sndr&& sndr) noexcept
            : rcvr(::std::forward<Rcvr>(rcvr)),
              inner_state(::beman::sequence::sequence_connect(::std::forward<Sndr>(sndr), receiver{this})) {}
    ...
    struct sender {
        ...
        auto connect(Rcvr&& rcvr) const& noexcept {
            return state<Rcvr, const sndr_t&>{::std::forward<Rcvr>(rcvr), this->sndr};
        }
    };
