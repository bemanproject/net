...
struct iota_t {
    ...
    struct state {
        ...
        using sndr_t = decltype(::beman::sequence::set_next(std::declval<rcvr_t&>(),
                                                            ::beman::execution::just(std::declval<T>())));
        ...
        void start() & noexcept { this->start_next(); }
        void start_next() noexcept {
            if (first != last) {
                ::beman::execution::start(this->inner_state.emplace(
                    ::beman::sequence::set_next(this->rcvr, ::beman::execution::just(this->first++)), receiver{this}));
            } else {
