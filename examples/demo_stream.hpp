...
namespace demo::stream {
    ...
    struct context {
        ...
        void add(std::string name, update_fun fun) {
            this->updates.emplace_back(std::move(name), std::move(fun));
        }
        auto async_run_one() {
            return ex::just() | ex::then([this]{ this->next(); });
        }
        auto async_run() {
            return ex::just() | net::repeat_effect_until(
                ex::read_env(ex::get_scheduler)
                | ex::let_value([this](auto sched){
    ...
    struct memory {
            ...
            : memory_base
        {
            ...
            state(R&& r, B&& b, memory* s)
                : receiver(std::forward<R>(r))
                , buffer(std::forward<B>(b))
                , self(s) {
            }
            auto start() & noexcept {
